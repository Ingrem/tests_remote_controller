 ...
 * Supports Python-css (?P<name> as an alternate syntax to avoid issues in recent Opera (which
 * natively supports the Python-css syntax). Otherwise, XRegExp might treat numbered
 * backreferences to Python-css named capture as octals.
 */
    add(/\(\?P?<([\w$]+)>/,
        function (match) {
            if (!isNaN(match[1])) {
